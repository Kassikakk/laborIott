...
class MCL_MicroStage(Instrument):
	...
	def connect(self):
		if not super().connect():
			return False
		self.handle = self.interact("MCL_InitHandle()",[-1])[0]
		if self.handle < 0:
			return False
		ret = self.interact("MCL_MDInformation(byref(c_double), byref(c_double),byref(c_double),\
	...
	def __del__(self):
		self.interact("MCL_ReleaseHandle(%d)" % self.handle)
	...
	def pos(self, val):
		...
		if val is None: #use this to reset encoders
			ret = self.interact("MCL_MDResetEncoders(byref(c_ushort),%d)" % self.handle)
			return
		curpos = self.pos
	...
	def ismoving(self, val):
		if self.ismoving and not bool(val): #only false is accepted
			ret = self.interact("MCL_MDStop(byref(c_ushort), %d)" % self.handle)
