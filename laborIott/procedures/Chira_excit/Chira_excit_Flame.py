...
from laborIott.instruments.Flame.VInst.FlameVI import Flame_VI
...
class ChiraExcit(*uic.loadUiType(localPath('Excit.ui'))):
	...
	def __init__(self):
		...
		#connect these to Flame as well
		self.startIdus.connect(self.flame.run)
		self.saveSpcData.connect(self.flame.saveData)
		...
		self.setExternalMode.connect(self.flame.setExternal)
	...
	def onStart(self):
		if self.scanning.is_set():
			...
			if self.autoFolderChk.isChecked():
				...
				if True: #I mean if we want the Flame to be switchable
					spcFolder = os.path.join(baseFolder, 'wl')
					os.mkdir(spcFolder)
					#ToDO: turn this to setSaveLoc later
					self.flame.saveLoc = spcFolder
					self.flame.locLabel.setText(spcFolder)
	...
	def scanProc(self, startwl, stopwl, stepwl, pwrTime, useSpc, usePwr):
		...
		while np.sign(stepwl) * curwl <= np.sign(stepwl) * stopwl:  # enable negative step
			...
			if useSpc:  # Spectrometer determines the time
				...
				while self.andor.dataQ.empty():
					if not self.scanning.is_set():
						return
				while self.flame.acquiring.is_set(): #also check that the Flame has finished
					if not self.scanning.is_set():
						return
				# idus shutter close?
			...
			if useSpc:  # save the spectral data
				...
				self.saveSpcData.emit(spcfilename) #this should save the Flame data as Well
